...
{
    if (dev_uuid == NULL) {
        LOGE("%s, Invalid device uuid", __func__);
        return;
    }
...
esp_err_t bluetooth_init(void)
{
    ...
    if (ret) {
        LOGE("%s initialize controller failed", __func__);
        return ret;
    }

    ret = esp_bt_controller_enable(ESP_BT_MODE_BLE);
    if (ret) {
        LOGE("%s enable controller failed", __func__);
        return ret;
    }
    ret = esp_bluedroid_init();
    if (ret) {
        LOGE("%s init bluetooth failed", __func__);
        return ret;
    }
    ret = esp_bluedroid_enable();
    if (ret) {
        LOGE("%s enable bluetooth failed", __func__);
        return ret;
    }
...
{
    LOGI("Resetting state; reason=%d", reason);
}
...
static void mesh_on_sync(void)
{
    ...
    if (rc != 0) {
        LOGI("error determining address type; rc=%d", rc);
        return;
    }
...
esp_err_t bluetooth_init(void)
{
    ...
    if (mesh_sem == NULL) {
        LOGE("Failed to create mesh semaphore");
        return ESP_FAIL;
    }
