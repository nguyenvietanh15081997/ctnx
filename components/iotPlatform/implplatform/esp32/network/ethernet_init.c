...
static esp_err_t spi_bus_init(void)
{
    ...
    if (ret != ESP_OK) {
        if (ret == ESP_ERR_INVALID_STATE) {
            IOT_LOGW(TAG, "GPIO ISR handler has been already installed");
            ret = ESP_OK; // ISR handler has been already installed so no issues
        } else {
            IOT_LOGE(TAG, "GPIO ISR handler install failed");
            goto err;
        }
    }
...
esp_err_t example_eth_init(esp_eth_handle_t *eth_handles_out[], uint8_t *eth_cnt_out)
{
    ...
    eth_handles = Mem_SafeCalloc(SPI_ETHERNETS_NUM + INTERNAL_ETHERNETS_NUM, sizeof(esp_eth_handle_t), "ethernet", "eth handles");
...
#endif // CONFIG_ETH_USE_SPI_ETHERNET
#else
    IOT_LOGD(TAG, "no Ethernet device selected to init");
